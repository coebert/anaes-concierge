/**
 * End-to-end test for the /api/chat POST handler when a full-day annual
 * leave overlaps a half-day leave in the same 90-day window.
 *
 * Per week (× 3 consecutive weeks):
 *   - Leave A (annual, full-day): Mon → Wed inclusive
 *   - Leave B (annual, half-day): Wed → Fri, `half_day_end = 'am'`
 *       covers Wed AM+PM, Thu AM+PM, Fri AM  (Fri PM stays)
 *   - Baseline rota: theatre AM+PM on Mon–Fri
 *
 * Expected weeklyGrid dominant-per-half-session:
 *   - Mon AM/PM  → On leave 3/3  (A only)
 *   - Tue AM/PM  → On leave 3/3  (A only)
 *   - Wed AM/PM  → On leave 3/3  (A ∩ B overlap must NOT double-label)
 *   - Thu AM/PM  → On leave 3/3  (B only)
 *   - Fri AM     → On leave 3/3  (B only)
 *   - Fri PM     → Main theatres 3/3 (half_day_end='am' preserves it)
 */
...
// Three consecutive Mon..Fri work weeks inside the 90-day window.
const WEEKS = [
...
// Overlapping approved leaves per week: full-day Mon→Wed and half-day
// Wed→Fri (end half = 'am' so Fri PM is preserved).
const pastApprovedLeave = WEEKS.flatMap((w) => [
  {
...
describe("/api/chat e2e — full-day + half-day leave overlap", () => {
  it("labels every covered half as 'On leave' and preserves Fri PM", async () => {
    ...
    // Every half from Mon..Thu (both sessions) plus Fri AM = "On leave" 3/3.
    for (const day of ["Mon", "Tue", "Wed", "Thu"] as const) {
      ...
      expect(am.location, `${day} AM`).toBe("On leave");
      expect(am.recurrence, `${day} AM recurrence`).toBe("3/3");
      expect(pm.location, `${day} PM`).toBe("On leave");
      expect(pm.recurrence, `${day} PM recurrence`).toBe("3/3");
    }

    const friAm = amRow.days.find((d) => d.weekday === "Fri")!;
    const friPm = pmRow.days.find((d) => d.weekday === "Fri")!;
    expect(friAm.location).toBe("On leave");
    expect(friAm.recurrence).toBe("3/3");
    // half_day_end='am' on Leave B must keep Fri PM as the original theatre.
    expect(friPm.location).toBe("Main theatres");
